...
第一版把工作区解析放在了**宿主路由**。于是出现一个必然的窗口期：**客户端半热重载、宿主半不重载** —— 页面调用了运行中的宿主**从没听过**的操作，真机报 `未知操作 "resolveWorkspace"`。**这不是「忘了重启」，是设计错误**：把一条**本来就不需要宿主**的解析放进了不重载的那一半。**判据**：凡是**客户端能拿到**的数据，就别让宿主参与 —— 宿主参与意味着一次重启的耦合，而重启窗口期内页面是坏的。
...
`contenteditable` 的渲染面 + 源码模式的 textarea，**三种模式各只有一个面**。比「镜像层」方案**更简单也更对**：没有「两层逐字对齐」的约束，因为**只有一层**。

**真正的难点是回写**（`HTML → Markdown` 是唯一会损坏提示词的方向）：**只在真实编辑时回写**（绑 `input`）—— 光是打开页面或切模式，绝不把文档过一遍 HTML；**只认渲染器自己产生的语义标签**，未知元素降级为文本而**不整块丢弃**；**粘贴强制纯文本**；**契约是结构保真不是字节相同**（`render(serialize(render(md))) === render(md)`）。

**已实测的一个真丢内容 bug**：`renderMarkdown` 产生的引用块是**裸文本**（没有段落包裹），而序列化器只遍历**元素**子节点 —— **整行引用被静默丢掉**。修法是给容器遍历加**裸文本回退**，修在通用位置。**单元套件测不到它**（替身 DOM 恰好建出带文本子节点的结构），**只有真浏览器装置才抓得到** —— 这正是留一个真装置的理由。
