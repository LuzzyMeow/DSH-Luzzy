...
> 更新时间：2026-09-23（第十四轮 · 前端 v2 全面升级）
> 用途：上下文压缩后的接续点。含**当前状态**、**失败史**、**下一步**。
> 配套：`AGENTS.md` §五「已证伪的做法」是防错清单（已扩到 5.29），本文件是进度与计划。
> 本轮新增：**前端 v2 —— 从「插件功能展示页」升级为「中文化智能体控制台」**（§二十三，末尾）。
> `src/client.js` 单文件 4274 行 → `src/` 下 28 个文件；五个控制台页面；新增只读路由
> `/__luzzy/runtime`。**宿主半有改动，需要重启 DSH 才生效。**
> 现状报告与迁移映射见 [`frontend-v2-analysis.md`](./frontend-v2-analysis.md) 与
> [`frontend-v2-migration.md`](./frontend-v2-migration.md)。
...
## 二十三、前端 v2：从「插件功能展示页」升级为「中文化智能体控制台」（第十四轮）

### 23.1 需求与交付

用户的方案书要求把 `LuzzyPage` 升级为**面向 Agent Harness 的中文化智能体控制台**，
按「分析现状 → 建立 UI 基础 → 重构页面架构 → 优先目标中心 → 执行状态 → Agent 配置 → 视觉优化 → 验收」实施。

| 交付 | 内容 |
|---|---|
| `docs/frontend-v2-analysis.md` | 动手前的现状报告：页面结构 / 组件结构 / 存在问题 / 重构范围 / 保留 / 删除六节 |
| `docs/frontend-v2-migration.md` | 迁移映射、数据层契约、构建链变更、验证命令、**七个真 bug 的记录**、未覆盖项 |
| `luzzy-page/src/` **28 个文件** | `app/`（router / app / dialog / manifest / frame.html）+ `pages/`（7）+ `components/`（9）+ `services/`（3）+ `styles/`（3） |
| `lib/runtime-routes.mjs` | **新增只读路由** `/__luzzy/runtime`：轮次 / 工具调用 / 生命周期 / 上下文 |
| 五个控制台页面 | 总览 / 目标中心 / 执行状态 / Agent 配置 / 系统信息（+ 预设 / 插件说明两个次要入口） |

### 23.2 两条硬约束下的技术选择

**① 帧文档必须是自包含单文档，`srcDoc` 不能有外部请求 —— 所以不能真用 `import`。**

做法是**源码多文件、产物单文档**：`build-font-css.py` 按 `src/app/manifest.json` 的显式顺序
把 22 个模块拼成帧内的一个 `<script>`。帧内模块用 IIFE 挂 `window.LZ` 命名空间，
**不用 `import`/`export`**——帧脚本是经典脚本，ESM 语法在那里直接是语法错误。

**② 帧文档原本是一个巨型模板字符串，反引号是地雷（踩过 4 次白屏）。**

改成 `json.dumps` 嵌成 **JSON 字符串字面量**，机械转义反引号 / 反斜杠 / `${`。
**这一类地雷在结构上消失了**，不再靠「记得转义」。

### 23.3 页面与数据的边界

```
后端响应  →  services/*-service.js  →  视图模型  →  app.js buildPageState  →  页面投影  →  pages/*.js（纯函数）
```

**判据**：页面里出现 `payload.` / `snapshot.` / `readError` / `LZ.App.state` 任一 = 越界。
`test-view-models.mjs` 逐页断言，并**当场抓出 `pages/system.js` 直读 usage 响应**——
修法是在 `buildPageState` 里投影，而不是放宽断言。

> **一条曾写错的断言**：第一版把 `delivery.acceptance` 也列为「后端字段」，于是两个页面
> 因为读 `view.delivery.acceptance` 被报越界。那是错的：视图模型**故意**暴露 `delivery`，
> 读它正是页面该做的事。分不清「原始形状」与「转换后形状」的断言会把正确代码报成坏的。

### 23.4 原样搬运，不重写

`components/{Format,Markdown,Chart}.js` 与 `app/dialog.js`、`pages/preset.js`（1164 行编辑器）
全部**按签名花括号配平逐字切出**，由 `tools/extract-legacy-modules.mjs` 与
`tools/extract-preset-module.mjs` 完成。它们承载 ~500 行测试覆盖的行为（单调插值、三种编辑模式、
序列化回写、帧内对话框），重写等于把测试全部作废。

搬运脚本的纪律：**每条替换都计数并打印**，末尾断言**没有任何未适配的帧全局引用残留**，
以及九个关键功能都在。预设编辑器还做了**去重**——三个纯函数与四个工具栏表原本落在搬运区间内部，
两处各得一份，删掉 `preset.js` 的副本。

### 23.5 本轮抓到的真 bug（**单元套件全绿时它们都存在**）

| # | 症状 | 根因 | 教训 |
|---|---|---|---|
| 1 | 帧完全空白 | 模块拼接缺分号 → `(A)(B)` 解析为函数调用 | **合法语法不等于正确行为**，解析门拦不住 |
| 2 | `"use strict" is not a function` | pragma 后无分号 → 被当成调用 | 编译测试对 ASI 问题毫无价值 |
| 3 | 切页即崩 | `Router.render` 传**整个应用状态**，页面按自己的契约读投影 | 投影自带 `tab` 才是结构上安全 |
| 4 | 系统信息页空 | `LZ.Activity.level` **是我编的**，真名是 `LZ.Format.levelOf` | 名字错在**渲染期**炸，不在加载期 |
| 5 | 预设页空 | 搬运只改调用点，没加模块绑定（`MD_ICONS` 等 13 处） | 局部绑定比逐点替换安全 |
| 6 | 任务树只显示 1 行 | 默认展开只看**直接子节点** | 「父已完成、孙还在做」的链被误判为全完成 |
| 7 | 预设页写着 `undefined` | 编辑器是命令式的（自己写 DOM 并返回 undefined），外壳用 `innerHTML =` 覆盖了它 | 外壳必须容忍两种渲染风格 |
| 8 | 53 个 CSS 类没有样式 | v2 重写新页面样式时**漏掉旧页面仍在用的部分** | 少一条选择器不报错，只有类名差分能发现 |
| 9 | `statusBlock is not defined` | 该函数**不在两个搬运区间的任何一个里** | 搬运边界要按「用到了什么」而不是「函数在哪」划 |

**第 1、2 条促成了构建的第四道门** `verify_frame_modules_evaluate`——它**运行**帧脚本，
而不是只解析。它自己的第一版还有个教训：**它把 `'use strict'` 从被求值的源码里切掉了**，
理由是「那是 harness 的 bug，不是帧的 bug」——**绕着可疑代码走，和不测它是一回事**。

**第 8 条的工具留下来了**：`tools/probe-unstyled-classes.mjs`（类名差分）与
`tools/recover-legacy-css.mjs`（从 git HEAD 逐字取回 114 条规则，不重写）。

### 23.6 验收

| 项 | 结果 |
|---|---|
| 全套件 | ✅ **28 / 28 绿**（基线 23 绿 + 1 既存 FAIL；本轮修好它并新增 4 个套件） |
| 三道帧门 | ✅ 全部重写为面向**组装后的帧**：完整性 9 条 / 结构 58 条 / 脚本从产物编译 5907 行 |
| 构建门 | ✅ 四道：依赖顺序 / 帧脚本编译 / **帧脚本运行** / 外层 bundle 编译 |
| 新套件 | ✅ `test-view-models` 58 条、`test-runtime-routes` 26 条（跑**真实日志**） |
| 真浏览器 | ✅ **7 / 7 页面**渲染，明暗两主题，控制台 0 报错 |
| 响应式 | ✅ 1280 / 720 / 420 三个宽度 `scrollWidth === innerWidth`（零横向溢出） |
| 截图 | ✅ **30 张，30 个互不相同的 sha256** |
| 目标中心 | ✅ 3104px 高、**13 节**（六节必需 + 7 节支撑）、任务树 5/5 节点、3 层深度 |
| 协作边界 | ✅ 后端 Goal 数据结构零改动；五个既有路由签名与状态码不变 |

**窄屏那个「溢出」是量出来才定案的**：420px 下 `.segment` 报溢出，实测
文档 `scrollWidth === innerWidth`（页面不横向滚动），tab 条是 `overflow-x: auto`，
滚动 94px 后最后一个 tab 坐标 464 < 492 —— **可达，不是缺陷**。
「页面不滚动」也可能是「有东西被裁掉了」，那是另一种缺陷，所以两层数字都打出来。

### 23.7 真机待办（需要用户）

- [ ] **重启 DSH**——宿主半新增了 `/__luzzy/runtime`，不重启则执行状态页显示「读不到」（如实说明，不是空白）
- [ ] 重启后在 DSH 里过一眼七个页面的观感（本轮的浏览器验收是在隔离装置里跑的）
- [ ] 确认新增的 tab 名称是「Luzzy 控制台」而非旧的「LuzzyPage」

### 23.8 还没做

- **图表交互的迁移后动态验证**：`render-hover-shot.mjs` 与 `test-animation.mjs` 仍指向**旧入口**
  （它们从旧渲染器提取帧），迁移后**未重跑**。悬停浮窗与切换动效的代码原样搬进了
  `components/Chart.js`，`test-chart-path.mjs`（曲线几何）仍绿，但「浮窗真的出现」这一层
  这轮没有动态证据。
- **AOCI 索引未收敛**：索引停在 v2 之前（受管范围内 `src/` 只有一个文件），
  且 19 张截图的策展裁决仍待人工。**没有伪造批次，也没有绕过那道门。**
